        ...
        {capturing && (
          <div className="absolute inset-0 flex items-center justify-center bg-black/50">
            <motion.div
              initial={{ scale: 0 }}
              animate={{ scale: 1 }}
              className="text-center bg-black/70 backdrop-blur-sm rounded-xl p-6"
            >
        ...
        <div className="max-w-2xl mx-auto">
          ...
          <div className="grid grid-cols-2 gap-4">
            <button
              ...
              className="bg-gradient-to-r from-purple-600 to-pink-600 text-white py-4 rounded-xl font-semibold disabled:opacity-50 disabled:cursor-not-allowed hover:shadow-lg transition-all"
            >
              {capturing ? (
          ...
          {capturedFrames.length > 0 && (
            <div className="mt-4 flex gap-2 overflow-x-auto pb-2">
              {capturedFrames.slice(-10).map((_, i) => (
                <div
                  key={i}
                  className="w-16 h-16 bg-green-500/30 border border-green-500 rounded flex-shrink-0 flex items-center justify-center text-white text-xs"
                >
                  ✓ {capturedFrames.length - 9 + i}
                </div>
              ))}
            </div>
          )}
